...
function NoduleDeviceCentral(uniqueId, name) {
    ...
    function startNoble() {
        ...
        noble.on('discover', function (peripheral) {
            ...
            var existing = _.find(devices, function (device) {
    ...
    function resolveProfile(device, deviceId, resolve, reject) {
        device.discoverAllServicesAndCharacteristics(function (error, services, characteristics) {
            if (error) {
                ...
                (function () {
                    var processCharacteristics = function processCharacteristics(service, characteristics, done) {
                        if (characteristics.length > 0) {
                            ...
                            characteristic.discoverDescriptors(function (error, descriptors) {
                                var descriptionDescriptor = _.find(descriptors, function (descriptor) {
                                    return descriptor.uuid == '2901';
                                });
                    ...
                    var processServices = function processServices(services) {
                            ...
                            var profile = {
                                ...
                                services: device.services.map(function (service) {
                                    return {
                                        ...
                                        }).reduce(function (memo, characteristic) {
                                            memo[characteristic.UUID] = characteristic;return memo;
                                        }, {})
                                    };
                                }).reduce(function (memo, service) {
                                    memo[service.UUID] = service;return memo;
                                }, {})
                            };
                            resolve(profile);
                        }
    ...
    this.connectPeripheral = function (deviceId) {
        return new Promise(function (resolve, reject) {
            var existingDevice = _.find(devices, function (device) {
                return device.uuid === deviceId;
            });
            if (existingDevice) {
                if (existingDevice.state == 'connected') {
                    ...
                    existingDevice.connect(function (error) {
                        if (error) {
                            reject(error);
                        } else {
                            resolveProfile(existingDevice, deviceId, resolve, reject);
    ...
    function execute(peripheralId, serviceId, characteristicId, command) {
        return new Promise(function (resolve, reject) {
            ...
            if (device) {
                var service = _.find(device.services, function (service) {
                    return service.UUID == serviceId;
                });
                if (service) {
                    var characteristic = _.find(service.characteristics, function (characteristic) {
